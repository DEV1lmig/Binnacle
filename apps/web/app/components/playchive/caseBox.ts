...
function buildFront(look: CaseLook) {
  if (look.from) {
    const front = look.from.cloneNode(true) as HTMLElement;
    front.classList.add("pk-c-lid-front");
    delete front.dataset.caseFlight;
    delete front.dataset.tilt;
    front.removeAttribute("id");
    // The clone is many times the shelf's size: let the browser pick a sharper candidate.
    for (const img of front.querySelectorAll("img")) { if (img.sizes) img.sizes = "1080px"; }
    return front;
  }
