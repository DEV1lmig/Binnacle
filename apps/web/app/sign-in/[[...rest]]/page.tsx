...
/**
 * Presents the customized Clerk Elements powered sign-in flow with Binnacle design.
 */
export default function SignInPage() {
  return (
    <main className="flex min-h-screen items-center justify-center bg-[var(--bkl-color-bg-primary)] px-4 py-16">
      <div className="grid w-full max-w-5xl overflow-hidden rounded-2xl border border-[var(--bkl-color-border)] bg-[var(--bkl-color-bg-primary)] shadow-[var(--bkl-shadow-2xl)] lg:grid-cols-[minmax(0,1fr)_minmax(0,420px)]">
        <section className="hidden flex-col justify-between border-r border-[var(--bkl-color-border)] bg-gradient-to-br from-[var(--bkl-color-accent-primary)]/20 via-[var(--bkl-color-accent-primary)]/10 to-transparent p-12 lg:flex relative overflow-hidden">
          {/* Gradient orb effect */}
          <div className="absolute top-0 right-0 w-96 h-96 bg-[var(--bkl-color-accent-primary)] rounded-full blur-[120px] opacity-20" />
          ...
          <div className="flex flex-col gap-6 relative z-10">
            ...
            <h1 className="text-4xl font-bold leading-tight text-[var(--bkl-color-text-primary)]">
              Welcome back to your gaming journey.
            </h1>
            <p className="max-w-sm text-base text-[var(--bkl-color-text-secondary)]">
              Track your backlog, share reviews, and discover what your friends are playing next.
            </p>
          </div>
          ...
          <ul className="flex flex-col gap-4 text-sm text-[var(--bkl-color-text-secondary)] relative z-10">
            ...
            <li className="flex items-start gap-3">
              <span className="mt-1 inline-flex h-2 w-2 flex-shrink-0 rounded-full bg-[var(--bkl-color-accent-primary)]" />
              Follow friends and discover their recommendations
            </li>
            <li className="flex items-start gap-3">
              <span className="mt-1 inline-flex h-2 w-2 flex-shrink-0 rounded-full bg-[var(--bkl-color-accent-primary)]" />
              Get insights on your gaming habits and stats
            </li>
          </ul>
        </section>
        ...
        <div className="flex flex-col gap-8 p-8 sm:p-12">
          <header className="flex flex-col gap-2 text-left lg:hidden">
            ...
            <h1 className="text-3xl font-bold text-[var(--bkl-color-text-primary)]">Welcome back</h1>
            <p className="max-w-md text-base text-[var(--bkl-color-text-secondary)]">
              Sign in to access your backlog and reconnect with your crew.
            </p>
          </header>
            ...
            <SignIn.Step name="start" className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-4">
                <Clerk.Field name="identifier">
                  <Clerk.Label className="text-sm font-medium text-[var(--bkl-color-text-secondary)]">Email or username</Clerk.Label>
                  <Clerk.Input asChild>
                ...
                <Clerk.Field name="password">
                  <div className="flex items-center justify-between text-sm font-medium text-[var(--bkl-color-text-secondary)]">
                    <Clerk.Label>Password</Clerk.Label>
                    <SignIn.Action navigate="forgot-password" className="text-[var(--bkl-color-accent-primary)] hover:text-[var(--bkl-color-accent-hover)] transition-colors">
                      Forgot?
                    </SignIn.Action>
                  </div>
                  <Clerk.Input asChild>
              ...
              <SignIn.Strategy name="email_code">
                <div className="flex flex-col gap-4">
                  <div className="flex flex-col gap-2">
                    <h2 className="text-xl font-semibold text-[var(--bkl-color-text-primary)]">Check your email</h2>
